...
4. 画像一覧で `offsetX` / `offsetY` を数値入力または±1ボタンで調整します。通常追加・分割追加の全Sprite Itemが共通の `offsetX` / `offsetY` を持ち、同じ描画計算・JSON出力を使います。初期値は0、単位は倍率適用後の出力pxです。UIでは右・上が正ですが、Project / Runtime JSONの内部値は従来どおり右・下が正で、プレビューへ即時反映します。
