...
(function (root) {
  ...
  test('LINE入力値validationと描画可能性validationを分離',()=>{
    const edit={...L.DEFAULT_EDIT,offsetX:3};equal(L.validateEdit(edit),edit);equal(L.validateSpecial({...L.DEFAULT_SPECIAL,offsetY:-4},'Main').offsetY,-4);
    for(const value of [1.5,1001,NaN])throws(()=>L.validateEdit({...edit,offsetX:value}));throws(()=>L.validateEdit({...edit,scale:10}));throws(()=>L.validateEdit({...edit,margin:41}));
    const bounds={x:0,y:0,width:100,height:80};throws(()=>LI.stickerPlan(bounds,{...L.DEFAULT_EDIT,scale:1.1}));throws(()=>LI.stickerPlan(null,L.DEFAULT_EDIT));throws(()=>LI.fixedPlan(bounds,240,240,{...L.DEFAULT_SPECIAL,offsetX:1},'Main Image'));
    equal(L.validateEdit({...L.DEFAULT_EDIT,scale:1.1}).scale,1.1);equal(L.validateSpecial({...L.DEFAULT_SPECIAL,offsetX:1},'Main').offsetX,1);
  });
